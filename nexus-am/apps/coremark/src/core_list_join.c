...
are divided as follows:
        ...
        o Bit 7 indicates if the lower 7 bits are to be used as is or
calculated.
...
list_head* core_list_init(ee_u32 blksize, list_head* memblock, ee_s16 seed)
{
    ...
    ee_u32 size     = (blksize / per_item) - 2; /* to accomodate systems with 64b pointers, and make sure
    ...
    for (i = 0; i < size; i++) {
        ...
        info.data16   = (dat << 8) | dat;          /* fill the data with actual data and upper bits with
                                                            rebuild value */
        core_list_insert_new(list, &info, &memblock, &datablock, memblock_end, datablock_end);
    }
    ...
    while (finder->next != NULL) {
        ...
        else
        {
            ...
            finder->info->idx = 0x3fff & (((i & 0x07) << 8) | pat); /* make sure the mixed items end
                                                                            up after the ones in sequence
                                                                            */
        }
        finder = finder->next;
    }
...
list_head *core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed) {
  ...
  while (finder->next != NULL) {
    if (i < size / 5) /* first 20% of the list in order */
      ...
      finder->info->idx =
          0x3fff & (((i & 0x07) << 8) |
                    pat); /* make sure the mixed items end
                                          up after the ones in sequence
                                          */
    }
    finder = finder->next;
...
/* Function: core_list_mergesort
   ...
   rather then recursive algorithm.
        The sort can either return the list to original order (by idx) ,
        or use the data item to invoke other other algorithms and change the
   order of the list.
