...
ImFont* ImFontAtlas::AddFont(const ImFontConfig* font_cfg)
{
    ...
    else
    {
        IM_ASSERT(!Fonts.empty());  // When using MergeMode make sure that a font has
                                    ...
                                    // ImGui::AddFontDefault() to add the default
                                    // imgui font.
    }
...
bool ImFontAtlas::Build()
{
    ...
    TexWidth = (TexDesiredWidth > 0)
                   ? TexDesiredWidth
                   : (total_glyph_count > 4000) ? 4096 : (total_glyph_count > 2000) ? 2048 : (total_glyph_count > 1000)
                                                                                                 ? 1024
                                                                                                 : 512;
    TexHeight                         = 0;
...
{
    if (!text_end) text_end = text_begin + strlen(text_begin);  // ImGui functions generally
