...
_PTR _DEFUN(_malloc_r, (ptr, nbytes),
            struct _reent *ptr _AND size_t nbytes) /* get a block */
{
...
_PTR _DEFUN(_realloc_r, (ptr, ap, nbytes),
            struct _reent *ptr _AND _PTR ap _AND size_t nbytes) {
...
_PTR _DEFUN(_realloc_r, (ptr, ap, nbytes),
  ...
  p = (char *)ap - ALIGN(8);
  if (p[1] < 13)
    onb = ((unsigned short *)p)[1] -
          (OVERHEAD - 1); /* old number of data bytes only */
  else
