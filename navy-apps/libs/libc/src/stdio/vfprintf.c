...
<<<<<<< HEAD
static int out_string(s, n_chars, flags, n_zero, n_pad, prefix, stream) char* s;
...
int _DEFUN(VFPRINTF, (stream, format, args),
           FILE *stream _AND _CONST char *format _AND va_list args) {
  return _VFPRINTF_R(stream->_data, stream, format, args);
}

int _DEFUN(_VFPRINTF_R, (data, stream, format, args),
           struct _reent *data _AND FILE *stream _AND _CONST char *format _AND
               va_list args) {
  char buffer[CVT_BUF_SIZE];
  int count = 0;
  int flags = 0;
...
=======
      ...
      if ((n_chars = out_string(fld, n_chars, flags, n_zero, n_pad, prefix,
                                stream)) == -1)
        return -1;
      else
        count += n_chars;
    ...
    case 'G': {
      ...
      fld =
          _dcvt(data, buffer, d, precision, field_width, f_type, flags & FHASH);
      ...
      if ((n_chars =
               out_string(fld, n_chars, flags, 0, n_pad, prefix, stream)) == -1)
        return -1;
      else
        count += n_chars;
    ...
    case 'p':
      ...
      if ((n_chars = out_string(fld, n_chars, flags, n_zero, 0, prefix,
                                stream)) == -1)
        return -1;
      else
        count += n_chars;
