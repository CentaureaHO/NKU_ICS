...
static _CONST char *_CONST dname[7] = {"Sunday",    "Monday",   "Tuesday",
...
size_t _DEFUN(strftime, (s, maxsize, format, tim_p),
              char *s _AND size_t maxsize _AND _CONST char *format _AND
  ...
  for (;;) {
    ...
    case 'c':
      if (count < maxsize - 24) {
        ...
        sprintf(&s[count], " %.2d %2.2d:%2.2d:%2.2d %.4d", tim_p->tm_mday,
    ...
    case 'W':
      if (count < maxsize - 2) {
        sprintf(&s[count], "%2.2d",
                (tim_p->tm_yday + ((8 - tim_p->tm_wday) % 7)) / 7);
        count += 2;
      } else
        return 0;
      break;
    ...
    case 'X':
      if (count < maxsize - 8) {
        sprintf(&s[count], "%2.2d:%2.2d:%2.2d", tim_p->tm_hour, tim_p->tm_min,
                tim_p->tm_sec);
        count += 8;
      } else
        return 0;
