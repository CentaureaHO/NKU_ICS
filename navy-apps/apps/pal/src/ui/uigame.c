...
--*/
{
  ...
  return (wReturnValue == MENUITEM_VALUE_CANCELLED || wReturnValue == 0) ? FALSE
                                                                         : TRUE;
}
...
--*/
{
  ...
  while (TRUE) {
    wMagic = PAL_MagicSelectionMenu(gpGlobals->rgParty[w].wPlayerRole, FALSE,
                                    wMagic);
    if (wMagic == 0) {
      break;
    }
...
--*/
{
  ...
  while (iCurrent >= 0 && iCurrent <= gpGlobals->wMaxPartyMemberIndex) {
    ...
    //
    PAL_DrawText(PAL_GetWord(STATUS_LABEL_EXP), PAL_XY(6, 6), MENUITEM_COLOR,
                 TRUE, FALSE);
    PAL_DrawText(PAL_GetWord(STATUS_LABEL_LEVEL), PAL_XY(6, 32), MENUITEM_COLOR,
                 TRUE, FALSE);
    PAL_DrawText(PAL_GetWord(STATUS_LABEL_HP), PAL_XY(6, 54), MENUITEM_COLOR,
                 TRUE, FALSE);
    PAL_DrawText(PAL_GetWord(STATUS_LABEL_MP), PAL_XY(6, 76), MENUITEM_COLOR,
                 TRUE, FALSE);
...
--*/
{
  ...
  while (TRUE) {
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
      ...
      PAL_DrawText(PAL_GetWord(gpGlobals->g.PlayerRoles
    ...
    while (TRUE) {
      ...
      if (SDL_GetTicks() > dwColorChangeTime) {
        ...
        PAL_DrawText(
            ...
            PAL_XY(125, 16 + 20 * wSelectedPlayer), bSelectedColor, FALSE,
            TRUE);
      }
