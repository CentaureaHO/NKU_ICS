...
<<<<<<< HEAD
    ...
    if (g_Battle.UI.fAutoAttack && !gpGlobals->fAutoBattle) {
        ...
        {
            PAL_DrawText(PAL_GetWord(BATTLEUI_LABEL_AUTO), PAL_XY(280, 10), MENUITEM_COLOR_CONFIRMED, TRUE, FALSE);
        }
    }
    ...
    //
    {
      ...
      for (i = 0; i < 4; i++) {
        if (g_Battle.UI.wSelectedAction == i) {
          PAL_RLEBlitToSurface(
              PAL_SpriteGetFrame(gpSpriteUI, rgItems[i].iSpriteNum), gpScreen,
              rgItems[i].pos);
      ...
      case kBattleMenuMain:
        if (g_InputState.dwKeyPress & kKeySearch) {
            ...
            do {
              ...
              if (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex]
                ...
                for (i = 0; i < MAX_INVENTORY; i++) {
                  if (gpGlobals->rgInventory[i].wItem ==
                    ...
                    break;
                             ...
                             .action.ActionType == kBattleActionUseItem) {
                if (gpGlobals->g
                ...
                (gpGlobals->g.PlayerRoles
                         .rgwHP[gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex]
                                    .wPlayerRole] == 0 ||
          ...
          if (j != -1) {
            g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].flTimeMeter =
                flMin - int2F(99);
            g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].state = kFighterWait;
            g_Battle.UI.state = kBattleUIWait;
          }
