...
--*/
{
  ...
  if (i >= 0 && i <= g_Battle.wMaxEnemyIndex &&
      g_Battle.rgEnemy[i].wObjectID != 0 && g_Battle.rgEnemy[i].e.wHealth > 0) {
    return i;
  }
...
--*/
{
  ...
  if (wAttackStrength > wDefense) {
    sDamage = F2int(int2F(wAttackStrength << 1) -
                    F_mul_int(f2F(1.6), wDefense) + f2F(0.5));
  } else if (wAttackStrength > F2int(F_mul_int(f2F(0.6), wDefense))) {
    sDamage = F2int(int2F(wAttackStrength) - F_mul_int(f2F(0.6), wDefense) +
                    f2F(0.5));
  } else {
    sDamage = 0;
  }
...
--*/
{
  ...
  //
  wMagicStrength =
      F2int(F_mul_int(RandomFloat(int2F(10), int2F(11)), wMagicStrength));
  wMagicStrength /= 10;
  ...
  if (gpGlobals->g.lprgMagic[wMagicID].wElemental != 0) {
    ...
    if (wElem <= NUM_MAGIC_ELEMENTAL) {
      sDamage *= 10 +
                 gpGlobals->g.lprgBattleField[gpGlobals->wNumBattleField]
...
--*/
{
  ...
  if (gpGlobals->bBattleSpeed > 1) {
    ...
    g_Battle.flTimeChargingUnit =
        F_div_F(g_Battle.flTimeChargingUnit, f2F(1.2));
  }
}
      ...
      if (i > gpGlobals->wMaxPartyMemberIndex) {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
          ...
          if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] == 0 ||
            ...
            //
            // players who are unable to move should attack physically if
            // recovered
            // in the same turn
            //
            ...
            case kBattleActionMagic:
              if ((gpGlobals->g.rgObject[g_Battle.rgPlayer[i].action.wActionID]
                       .magic.wFlags &
        ...
        for (j = 0; j < MAX_POISONS; j++) {
          if (gpGlobals->rgPoisonStatus[j][i].wPoisonID != 0) {
            gpGlobals->rgPoisonStatus[j][i].wPoisonScript =
                PAL_RunTriggerScript(
                    gpGlobals->rgPoisonStatus[j][i].wPoisonScript, wPlayerRole);
          }
        }
...
--*/
{
  ...
  case kBattleActionMagic:
    ...
    if (gpGlobals->g.PlayerRoles.rgwMP
            [gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex].wPlayerRole] < w) {
      w = g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID;
      w = gpGlobals->g.lprgMagic[gpGlobals->g.rgObject[w].magic.wMagicNumber]
              .wType;
...
--*/
{
  ...
  index =
      gpGlobals->g.rgwBattleEffectIndex[PAL_GetPlayerBattleSprite(wPlayerRole)]
                                       [1];
  index *= 3;
  ...
  for (i = 0; i < 3; i++) {
    ...
    PAL_RLEBlitToSurface(b, gpScreen, PAL_XY(x - PAL_RLEGetWidth(b) / 2,
...
--*/
{
  ...
  if (!fSummon) {
    ...
    index =
        gpGlobals->g
            .rgwBattleEffectIndex[PAL_GetPlayerBattleSprite(wPlayerRole)][0];
    index *= 10;
    index += 15;
#ifdef PAL_WIN95
    SOUND_Play(gpGlobals->g.PlayerRoles.rgwMagicSound[wPlayerRole]);
  ...
  for (i = 0; i < n; i++) {
    ...
    if (gpGlobals->g.lprgMagic[iMagicNum].wType == kMagicTypeApplyToParty) {
      ...
      if (sTarget > 0 && g_Battle.iHidingTime == 0) {
        if (gpGlobals
                ->rgPlayerStatus[gpGlobals->rgParty[sTarget - 1].wPlayerRole]
                                [kStatusConfused] == 0) {
          LPCBITMAPRLE p =
      ...
      if (sTarget == -1) {
        ...
        sDamage /=
            ((g_Battle.rgPlayer[sTarget].fDefending ? 2 : 1) *
             ((gpGlobals->rgPlayerStatus[wPlayerRole][kStatusProtect] > 0)
                  ? 2
                  : 1)) +
            (fAutoDefend ? 1 : 0);
