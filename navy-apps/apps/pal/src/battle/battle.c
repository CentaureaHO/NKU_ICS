...
<<<<<<< HEAD
    ...
    for (i = g_Battle.wMaxEnemyIndex; i >= 0; i--) {
        ...
        pos = PAL_XY(
            PAL_X(pos) -
                PAL_RLEGetWidth(PAL_SpriteGetFrame(g_Battle.rgEnemy[i].lpSprite, g_Battle.rgEnemy[i].wCurrentFrame)) /
                    2,
            PAL_Y(pos) -
...
=======
  ...
  for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
    ...
    if (fLevelUp) {
      ...
      PAL_DrawNumber(OrigPlayerRoles.rgwDexterity[w] +
                         PAL_GetPlayerDexterity(w) -
