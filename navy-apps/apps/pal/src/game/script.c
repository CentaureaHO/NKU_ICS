...
--*/
{
  LPCSTR rgszStrings[] = {
      ...
      "                    (" __DATE__ ")",
#endif
      " ", "  (c) 2009-2014, Wei Mingzhi", "      <whistler_wmz@users.sf.net>.",
#ifdef __SYMBIAN32__
...
#endif
      " ", "\xA5\xBB\xB5\x7B\xA6\xA1\xAC\x4F\xA6\xDB\xA5\xD1\xB3\x6E\xC5\xE9"
           "\xA1\x41\xAB\xF6\xB7\xD3"
           " GNU General",
...
--*/
{
  ...
  case 0x001B:
    ...
    if (pScript->rgwOperand[0]) {
      ...
      //
      // Apply to one player. The wEventObjectID parameter should indicate the
      // player role.
      //
      ...
      for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        if (gpGlobals->g.PlayerRoles.rgwHP[w] == 0) {
          gpGlobals->g.PlayerRoles.rgwHP[w] =
              gpGlobals->g.PlayerRoles.rgwMaxHP[w] * pScript->rgwOperand[1] /
              10;
  ...
  case 0x0023:
    ...
    if (pScript->rgwOperand[1] == 0) {
      ...
      w = gpGlobals->g.PlayerRoles.rgwEquipment[pScript->rgwOperand[1] - 1]
                                               [iPlayerRole];
      if (w != 0) {
        PAL_RemoveEquipmentEffect(iPlayerRole, pScript->rgwOperand[1] - 1);
        PAL_AddItemToInventory(w, 1);
        gpGlobals->g.PlayerRoles.rgwEquipment[pScript->rgwOperand[1] - 1]
                                             [iPlayerRole] = 0;
      }
    }
    break;
  ...
  case 0x0030:
    ...
    {
      WORD *p =
          (WORD *)(&gpGlobals->rgEquipmentEffect[kBodyPartExtra]); // HACKHACK
  ...
  case 0x0051:
    ...
    PAL_FadeIn(gpGlobals->wNumPalette, gpGlobals->fNightPalette,
               ((SHORT)(pScript->rgwOperand[0]) > 0) ? pScript->rgwOperand[0]
                                                     : 1);
...
//
  ...
  case 0x0077:
    ...
    //
    PAL_PlayMUS(0, FALSE, (pScript->rgwOperand[0] == 0)
                              ? 2.0f
    ...
    if (pScript->rgwOperand[0] == 0 && pScript->rgwOperand[1] == 0) {
      ...
      do {
        if (pScript->rgwOperand[2] == 0xFFFF) {
          ...
          gpGlobals->viewport = PAL_XY(PAL_X(gpGlobals->viewport) + x,
                                       PAL_Y(gpGlobals->viewport) + y);
          gpGlobals->partyoffset = PAL_XY(PAL_X(gpGlobals->partyoffset) - x,
  ...
  case 0x0081:
    ...
    //
    {
      ...
      if (abs(x) + abs(y * 2) < pScript->rgwOperand[1] * 32 + 16) {
        if (pScript->rgwOperand[1] > 0) {
          //
          // Change the trigger mode so that the object can be triggered in next
          // frame
          //
  ...
  case 0x0083:
    //
    // Jump if event object is not in the specified zone of the current event
    // object
    //
    if (pScript->rgwOperand[0] <=
  ...
  while (wScriptEntry != 0 && !fEnded) {
    ...
    case 0x0005:
      ...
      if (PAL_DialogIsPlayingRNG()) {
        ...
        VIDEO_UpdateScreen(NULL);
        UTIL_Delay(
            (pScript->rgwOperand[1] == 0) ? 60 : (pScript->rgwOperand[1] * 60));
      }
