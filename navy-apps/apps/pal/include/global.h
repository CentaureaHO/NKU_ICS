...
typedef struct tagSCENE
{
    ...
    WORD wScriptOnTeleport;  // when teleporting out of this scene, execute script
                             // from here
...
typedef struct tagOBJECT_PLAYER
{
    ...
    WORD wScriptOnFriendDeath;  // when friends in party dies, execute script from
                                // here
...
typedef struct tagSTORE { WORD rgwItems[MAX_STORE_ITEM]; } STORE, *LPSTORE;
...
typedef struct tagPLAYERROLES
{
    ...
    WORD    rgwElementalResistance[NUM_MAGIC_ELEMENTAL][MAX_PLAYER_ROLES];  // resistance to elemental
                                                                            // magics
