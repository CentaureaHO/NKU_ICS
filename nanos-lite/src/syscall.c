...
#define X(name, idx, done) \
    case SYS_##name: panic("Not implemented function for %s in navy-apps/libs/libos/src/nanos.c", #name);
        SYSCALLS
#undef X
        default: panic("Unhandled syscall ID = %d", target);
    }
...
#define X(name, idx, done) \
    case SYS_##name: HANDLER_##done(name)
        SYSCALLS
        default: panic("Unhandled syscall ID = %d", call_num);
    }
